...
const inch = 72, // 72 dpi
  ...
  tabMargin = 1.2, // minimum space to leave between tabs
  tolerance = 0.01, // padding;
  th_top = 0.2 + tolerance,
...
function drawLetter(letter, x, y) {
  ...
  var polygon = draw.polygon(new SVG.PointArray(arr)).move(xi, yi).fill('none').stroke({
    opacity: 0.8,
    width: 0.5
  })
}
...
function addTabs(arr, x, y) {
  ...
  for (var i = 0; i < arr.length; ++i) {
    ...
    var leftPad = (i == 0) ? false : -1 == cross(prevNorm, norm) // padding on side
    prevNorm = norm;
...
function drawSide(panel, x, y) {
  var nt = panel.nt,
    wd = panel.wd,
    lp = panel.lp,
    lDelta = lp ? th_side : -th_side, // if leftPad is true, extend one of the sides, if false, move the side in
    p1 = [-wd / 2, side_depth],
    p2 = [wd / 2, side_depth],
    norm = [0, -1], // normal vector is in downward direction
    edge = [1, 0], // edge vector
    pts = [p1, [wd / 2 + lDelta, side_depth]];

  pts.splice(1, 0, ...getTabPts(nt, norm, edge, p1, p2, th_top));

  var p1 = [wd / 2, 0],
    p2 = [-wd / 2, 0],
    norm = [0, 1], // normal vector is in upward direction
    edge = [-1, 0], // edge vector
    pts2 = [
      [wd / 2 + lDelta, 0], p2
    ];
  ...
  var polygon = draw.polygon(new SVG.PointArray(arr)).fill('none').stroke({
    opacity: 0.8,
    width: 0.5
  })
}
...
function drawI(w, h) { // generates points on a basic I
  // all units in inches
  var t1 = 3, // horizontal thickness
    t2 = 2.5; // vertical thickness
  var base = [
    [0, h / 2],
    [w / 2, h / 2],
    [w / 2, h / 2 - t2],
